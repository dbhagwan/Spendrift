...
/// Aurora backdrop: soft accent glows over deep black (dark) or warm
/// off-white (light). Glass surfaces need color behind them to refract —
/// this is what makes the blur read as glass instead of flat gray.
struct AppBackground: View {
    ...
    var body: some View {
        ...
        ZStack {
            (dark ? Color.black : Color(red: 0.95, green: 0.96, blue: 0.955))
            Circle()
                .fill(Theme.accent.opacity(dark ? 0.30 : 0.16))
                .frame(width: 430, height: 430)
                .blur(radius: 120)
                .offset(x: -130, y: -330)
            Circle()
                .fill(Color(red: 0.16, green: 0.34, blue: 0.58).opacity(dark ? 0.32 : 0.14))
                .frame(width: 400, height: 400)
                .blur(radius: 110)
                .offset(x: 170, y: -140)
            Circle()
                .fill(Theme.accent.opacity(dark ? 0.12 : 0.08))
...
/// (iOS 26) over the aurora backdrop. `.interactive()` makes the glass
/// respond to touch with light play, part of the app feeling alive.
